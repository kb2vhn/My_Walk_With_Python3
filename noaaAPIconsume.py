...
n = noaa.NOAA()
res = n.get_forecasts('14810', 'US', False) # I wanted the detailed forcast and the api.weather.gov has empty detailedForecast
# on the hour by hour
for i in res:
    # print(i)
    print(i['name'], re.findall('([0-9]?[0-9]-[0-9]?[0-9])T([0-2]?[0-9]:[0-5]?[0-9])', i['startTime']), 
    'Temp', i['temperature'], 'Wind', i['windSpeed'], 'Direction', i['windDirection'], 
    '\nForecast', i['detailedForecast']) # I did a regex on the start time otherwise it looks like this
